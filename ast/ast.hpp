...
/*!	
  \class AlphaNumericNode
  \brief Definition of atributes and methods of NumberNode class
  \note  NumberNode Class publicly inherits from ExpNode class
*/
class AlphaNumericNode : public ExpNode 
{
 private: 	
   char * _string; //!< \brief string of the AlphaNumericNode
...
class RepeatTillStmt : public Statement 
{
 private:
  ExpNode *_cond; //!< Condicion of the while statement
  std::list<Statement *> *_stmt; //!< Statement of the body of the while loop
...
/*!		
	...
	\post  A new WhileStmt is created with the parameters
*/
  RepeatTillStmt(ExpNode *condition, std::list<Statement *> *statements)
	{
		this->_cond = condition;
		this->_stmt = statements;
	}
