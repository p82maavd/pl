	...
	if (this->_exp != NULL)
	{
		...
		switch(this->_exp->getType())
		{
			...
			case STRING:
			{
				...
				char* value;
				// evaluate the expression as BOOL
			 	value = dynamic_cast<AlphaNumericNode*>(this->_exp)->evaluateAlphaNumeric();
...
void lp::PrintStmt::evaluate() 
{
	...
	switch(this->_exp->getType())
	{
		...
		case BOOL:
			...
			break;
		case STRING:
				std::cout << dynamic_cast<AlphaNumericNode*>(this->_exp)->evaluateAlphaNumeric() << std::endl;
				break;
...
{   
	...
	{
		/* Get the identifier in the table of symbols as NumericVariable */
		//CAMBIAR NUMERIC VARIABLE por otra que sea de string, hay que crear un fichero nuevo, comprobar tambien Assigment
	...
	else
	{
		...
		//Esto creo que sobra pero bueno
		table.eraseSymbol(this->_id);
